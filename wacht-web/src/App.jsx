...
function CheckCard({ check }) {
  const up = check.status.includes('up')
  return (
...
export default function App() {
  ...
  const allUp = checks.length > 0 && checks.every(c => c.status.includes('up'))
  const downCount = checks.filter(c => !c.status.includes('up')).length
