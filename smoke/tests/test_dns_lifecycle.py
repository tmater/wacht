...
def test_dns_lifecycle(server, mock):
    ...
    token = server.login()
    check_id = f"smoke-dns-{uuid.uuid4().hex[:8]}"
    payload = {
        "id": check_id,
        "type": "dns",
        "target": DNS_TARGET,
    ...
    try:
        with cleanup.preserve_primary_error():
            ...
            assert_check_definition(server, token, check_id, expected_target=DNS_TARGET)
...
def assert_check_definition(server, token, check_id, expected_target):
    ...
    if len(checks) != 1:
        raise SmokeError(f"expected exactly 1 dns check row for {check_id}, got {len(checks)}")
    check = checks[0]
    if check.get("type") != "dns":
        raise SmokeError(f"expected dns check type for {check_id}, got {check}")
    if check.get("target") != expected_target:
        raise SmokeError(f"expected dns check target {expected_target!r} for {check_id}, got {check}")
...
def assert_incident_stable(server, token, check_id, seconds):
    ...
    while time.monotonic() < deadline:
        ...
        if opened is None:
            raise SmokeError(f"expected exactly 1 open incident for dns check {check_id} while down")
        time.sleep(1)
