...
def test_public_status_page(server, mock):
    ...
    check_id = f"smoke-public-{uuid.uuid4().hex[:8]}"
    payload = {
        "id": check_id,
        "type": "http",
    ...
    try:
        with cleanup.preserve_primary_error():
            pending = wait_for(
                ...
                timeout_seconds=30,
                interval_seconds=2,
                fn=lambda: public_status_for_check(server, slug, check_id, allowed_statuses={"pending", "up"}),
            )
            ...
            healthy = wait_for(
                ...
                timeout_seconds=60,
                interval_seconds=2,
                fn=lambda: public_status_for_check(server, slug, check_id, allowed_statuses={"up"}),
            )
            ...
            opened = wait_for(
                ...
                timeout_seconds=60,
                interval_seconds=2,
                fn=lambda: public_status_for_check(server, slug, check_id, allowed_statuses={"down"}, require_incident_since=True),
            )
...
def public_status_for_check(server, slug, check_id, *, allowed_statuses, require_incident_since=False):
    payload = server.get_public_status(slug)
    checks = {check["check_id"]: check for check in payload.get("checks", [])}
